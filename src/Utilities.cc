...
vector <OverlappedImages> Utilities::FindOverlappingImages(vector <Images> &images, vector <IncidenceAngles> &incidenceangles, vector <MasterImageOrientation> &orientations)
{
  /****************************************************/
  /* Declaration/Initialization of function variables */
  /****************************************************/
  ...
  /*****************************************************/
  /* Sort orientations vector based on "field2" column */
  /*****************************************************/
  SortMasterImageOrientation sortorientations(orientations, field2);
  sortorientations.arrange();
  orientations = sortorientations.GetMasterImageOrientations();
  //ReadAuxilaryData::Display(orientations); exit(0);
  ...
  for(int i=0; i<n1; i++)
  {
    ...
    /**************************************************/
    double value = images[i].GetX_origin()+1.1*images[i].GetPhysical_width();
...
void Utilities::ExecuteImagesAuxilaryFunction(vector <Images> &images, vector <ImageSets> &imagesets)
{
  ...
  for(int i=0; i<(int)imagesets.size(); i++)
  {
    ...
    /************************************************************************************/
    string upper_left_x = imagesets[i].GetUpper_left_longitude();
    string upper_left_y = imagesets[i].GetUpper_left_latitude();
    ...
    //double average_resolution = (horizontal_resolution+vertical_resolution)/2.0;
    //double max_resolution = (horizontal_resolution > vertical_resolution) ? horizontal_resolution : vertical_resolution;
    //double min_resolution = (horizontal_resolution < vertical_resolution) ? horizontal_resolution : vertical_resolution;

    /*****************************************************/
    /* Retrieve x and y offsets for current master image */
    /*****************************************************/
    double x_offset = imagesets[i].GetX_origin();
    double y_offset = imagesets[i].GetY_origin();
    ...
    /**************************************************************/
    /* Iterate through images vector and execute AuxilaryFunction */
    /**************************************************************/
    while(master_image_name.compare(images[index].GetName().substr(0, p1)) == 0)
    {
      images[index].AuxilaryFunction(upper_left_x, upper_left_y, pixel_resolution, horizontal_resolution, vertical_resolution, x_offset, y_offset);
...
void Utilities::ExecuteImageSetsAuxilaryFunction(vector <ImageSets> &imagesets, vector <MasterImageOrientation> &orientations)
{
  /**********************************************/
  /* Sort imagesets vector based on name column */
  /**********************************************/
  ...
  /*************************************************/
  /* Sort orientations vector based on name column */
  /*************************************************/
  SortMasterImageOrientation sortorientations(orientations, "name");
  sortorientations.arrange();
  orientations = sortorientations.GetMasterImageOrientations();
  ...
  for(int i=0; i<(int)imagesets.size(); i++)
  {
    string XORIGIN = to_upper_copy<string>(orientations[i].GetUpper_left_longitude());
    string YORIGIN = to_upper_copy<string>(orientations[i].GetUpper_left_latitude());
    if((XORIGIN.compare("MAX") == 0) || (XORIGIN.compare("MIN") == 0))
    {
      if((YORIGIN.compare("MAX") == 0) || (YORIGIN.compare("MIN") == 0))
	imagesets[i].AuxilaryFunction(XORIGIN, YORIGIN);
      else
	exit(EXIT_FAILURE);
    }
    else
